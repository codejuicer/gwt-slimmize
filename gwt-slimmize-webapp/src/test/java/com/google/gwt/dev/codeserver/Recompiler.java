...
public class Recompiler {
  ...
  /**
   * Summarizes the inputs to a GWT compile. (Immutable.) Two summaries should
   * be equal if the compiler's inputs are equal (with high probability).
   */
  ...
  private static String generateModuleRecompileJs(ModuleDef module, TreeLogger compileLogger)
    ...
    try {
      ...
      compileLogger.log(Type.ERROR, "Can not generate + " + outputModuleName + " recompile js",
          e);
      throw new UnableToCompleteException();
    }
  }

  private static boolean maybeOverrideConfig(ModuleDef module, String propName,
      String newValue) {
  ...
  /**
   * Attempts to set a binding property to the given value. If the value is not
   * allowed, see if we can find a value that will work. There is a special case
   * for "locale".
   *
  ...
  /**
   * Sets a binding even if it's set to a different value in the GWT
   * application.
   */
  private static void overrideBinding(ModuleDef module, String propName, String newValue) {
    BindingProperty binding = module.getProperties().findBindingProp(propName);
    if (binding != null) {
      // This sets both allowed and generated values, which is needed since the
      // module
      // might have explicitly disallowed the value.
      // It persists over multiple compiles but that's okay since we set it the
      // same way
      // every time.
  ...
  private static void writeRecompileNoCacheJs(File outputDir, String moduleName,
      String content,
  ...
  /**
   * The module name that the recompiler passes as input to the GWT compiler
   * (before renaming).
   */
  ...
  /**
   * The module name that the GWT compiler uses in compiled output (after
   * renaming).
   */
  ...
  /**
   * Creates a dummy output directory without compiling the module. Either this
   * method or {@link #precompile} should be called first.
   */
  synchronized Job.Result initWithoutPrecompile(TreeLogger logger)
    ...
    try {
      ...
      CompilerOptions loadOptions =
          new CompilerOptionsImpl(compileDir, inputModuleName, options);
      compilerContext =
          compilerContextBuilder.options(loadOptions).unitCache(unitCache).build();

      // Loads and parses all the Java files in the GWT application using the
      // JDT.
      // (This is warmup to make compiling faster later; we stop at this point
      // to avoid
    ...
    compileLogger.log(TreeLogger.Type.INFO,
        "Module setup completed in " + elapsedTime + " ms");

    return new Result(compileDir, module.getName(), null);
  }

  /**
   * Compiles the first time, while Super Dev Mode is starting up. Either this
   * method or {@link #initWithoutPrecompile} should be called first.
   */
   ...
   *
   * <p>
   * Prerequisite: either {@link #precompile} or {@link #initWithoutPrecompile}
   * should have been called first.
   *
   * <p>
   * Sets the job's result and returns normally whether the compile succeeds or
   * not.
   *
  ...
  /**
   * Calls the GWT compiler with the appropriate settings. Side-effect: a
   * MinimalRebuildCache for the current binding properties will be found or
   * created.
   *
  ...
  private boolean doCompile(TreeLogger compileLogger, CompileDir compileDir, Job job)
    ...
    // Check if the module definition + job specific binding property
    // restrictions expanded to more
    // than permutation.
    PropertyCombinations propertyCombinations =
        new PropertyCombinations(module.getProperties(), module.getActiveLinkerNames());
    List<PropertyCombinations> permutationPropertySets =
        propertyCombinations.collapseProperties();
    ...
    if (options.isIncrementalCompileEnabled()) {
      // Returns a copy of the intended cache, which is safe to modify in this
      // compile.
      minimalRebuildCache =
    ...
    if (success) {
      ...
      writeRecompileNoCacheJs(new File(publishedCompileDir.getWarDir(), moduleName),
          moduleName,
  ...
  /**
   * Loads the module and configures it for SuperDevMode. (Does not restrict
   * permutations.)
   */
  private ModuleDef loadModule(TreeLogger logger) throws UnableToCompleteException {
    ...
    if (SingleScriptLinker.class.isAssignableFrom(linker)) {
      logger.log(TreeLogger.INFO,
          "EXPERIMENTAL SingleScriptLinker Linker Found: "
              + linker.getName());
    } else if (!CrossSiteIframeLinker.class.isAssignableFrom(linker)) {
      logger.log(TreeLogger.ERROR,
          "linkers other than CrossSiteIFrameLinker aren't supported. Found: "
              + linker.getName());
      throw new UnableToCompleteException();
    }
    ...
    // Disable the redirect hook here to make sure we don't have an infinite
    // loop.
    ...
    if (config.getBoolean("installCode", true)) {
      ...
      // Make sure installScriptJs is set to the default for compiling without
      // installCode.
    ...
    // Fix bug with SDM and Chrome 24+ where //@ sourceURL directives cause
    // X-SourceMap header to be ignored
    ...
    if (options.isIncrementalCompileEnabled()) {
      // CSSResourceGenerator needs to produce stable, unique naming for its
      // input.
      // Currently on default settings CssResourceGenerator's obfuscation
      // depends on
      // whole world knowledge and thus will produce collision in obfuscated
      // mode, since in
      // incremental compiles that information is not available.
      //
      // TODO(dankurka): Once we do proper stable hashing of classes in
      // CssResourceGenerator, we
  ...
  /**
   * Restricts the compiled permutations by applying the given binding
   * properties, if possible. In some cases, a different binding may be chosen
   * instead.
   */
  ...
  /**
   * Prepares a stub compile directory. It will include all "public" resources
   * and a nocache.js file that invokes the compiler.
   */
  private void setUpCompileDir(CompileDir compileDir, ModuleDef module,
    ...
    try {
      ...
      File noCacheJs =
          new File(outputDir.getCanonicalPath(), module.getName() + ".nocache.js");
      Files.write(stub, noCacheJs, Charsets.UTF_8);

      // Create a "module_name.recompile.nocache.js" that calculates the
      // permutation
